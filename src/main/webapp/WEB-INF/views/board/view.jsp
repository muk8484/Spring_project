...
// 		alert("댓글 등록 이벤트");
	...
	$("#modalWriteReplyBtn").click(function(){
		...
		let memberWriter = $("#memberWriter").val();
		
		// 데이터가 비어 있으면 안된다. 체크
		if(!content || !(!writer || !memberWriter)){
		...
		// 서버에 데이터를 전달해서 등록한다.
		replyService.write(
			{content : content, writer : (writer ? writer : memberWriter), no : no},
			function(result){
				alert("RESULT : " + result);
...
//		alert("on 이벤트 전달- 수정 / 삭제");
		...
		$("#writer").val(writer);
		// 작성자는 수정 불가로 처리
		$("#memberWriter").attr("readonly","readonly");
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<textarea class="form-control" rows="5" id="content" required="required"></textarea>
					</div>
					<div class="form-group">
						...
						<c:choose>
							<c:when test="${!empty login }">
								<input type="text" class="form-control" id="memberWriter" required="required" value="${login.name }(${login.id })">
							</c:when>
							<c:otherwise>
								<input type="text" class="form-control" id="writer" required="required">
							</c:otherwise>
						</c:choose>
					</div>
				</div>
